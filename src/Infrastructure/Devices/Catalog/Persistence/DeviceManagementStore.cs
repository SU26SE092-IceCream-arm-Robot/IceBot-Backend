...
namespace Infrastructure.Devices.Catalog.Persistence;
...
public sealed class DeviceManagementStore : IDeviceManagementStore
{
    ...
    {
        var query = _dbContext.Devices.WhereNotDeleted()
    ...
    {
        return _dbContext.Devices.WhereNotDeleted()
            .Include(d => d.DeviceType)
            .Include(d => d.DeviceModel)
            .Include(d => d.Kiosk)
    ...
    public Task<Device?> GetByKioskIdAsync(
        Guid kioskId,
        Guid deviceId,
        CancellationToken cancellationToken = default)
    {
        return _dbContext.Devices.WhereNotDeleted()
            ...
            .Include(d => d.Kiosk)
            .FirstOrDefaultAsync(
                d => d.Id == deviceId && d.KioskId == kioskId,
                cancellationToken);
    }
    ...
    public Task<bool> DeviceModelIsAssignedAsync(
        Guid deviceModelId,
        CancellationToken cancellationToken = default) =>
        _dbContext.Devices.WhereNotDeleted().AnyAsync(device => device.DeviceModelId == deviceModelId, cancellationToken);
