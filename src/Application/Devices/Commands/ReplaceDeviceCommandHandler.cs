    ...
    IInventoryStore inventory)
{
    ...
    {
        var source = await devices.GetByIdAsync(command.SourceDeviceId, cancellationToken);
        var target = await devices.GetByIdAsync(command.Request.ReplacementDeviceId, cancellationToken);
        ...
        if (source.KioskId != target.KioskId)
        {
            return ApiResult<DeviceReplacementResult>.Fail("Replacement device must belong to the same kiosk.", 409);
        }
        ...
        source.Status = DeviceStatus.Retired;
