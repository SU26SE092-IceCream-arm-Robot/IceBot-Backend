...
internal static class ProductionIncidentResultMapper
{
    ...
    {
        Id = incident.Id, OrganizationId = incident.OrganizationId, StoreId = incident.StoreId,
        KioskId = incident.KioskId, OrderId = incident.OrderId, OrderItemId = incident.OrderItemId,
        OrderNumber = incident.OrderNumberSnapshot, ProductName = incident.ProductNameSnapshot,
        ProductVariantName = incident.ProductVariantNameSnapshot, SourceCommandId = incident.SourceCommandId,
        SourceProductionJobId = incident.SourceProductionJobId, ProductionUnitNo = incident.ProductionUnitNo,
        ProductionUnitQuantity = incident.ProductionUnitQuantity, Trigger = incident.Trigger.ToString(),
        Status = incident.Status.ToString(), PhysicalOutputState = incident.PhysicalOutputState.ToString(),
        InspectionOutcome = incident.InspectionOutcome?.ToString(), Resolution = incident.Resolution?.ToString(),
        ErrorCode = incident.ErrorCodeSnapshot, ErrorMessage = incident.ErrorMessageSnapshot,
        RelatedEdgeCommandId = incident.RelatedEdgeCommandId, RelatedRefundId = incident.RelatedRefundId,
        CreatedAt = incident.CreatedAt, InspectedAt = incident.InspectedAt, ResolvedAt = incident.ResolvedAt,
