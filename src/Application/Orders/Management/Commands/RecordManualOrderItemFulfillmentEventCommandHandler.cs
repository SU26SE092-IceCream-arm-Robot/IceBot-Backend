...
public sealed class RecordManualOrderItemFulfillmentEventCommandHandler(
    IOrderStore orders,
    IRealtimeNotificationPublisher publisher)
{
    public async Task<ApiResult<ManagementOrderDetailResult>> HandleAsync(
        RecordManualOrderItemFulfillmentEventCommand command,
        CancellationToken cancellationToken = default)
    {
        if (command.OrderId == Guid.Empty || command.OrderItemId == Guid.Empty ||
            command.Request.FulfillmentEventId == Guid.Empty || command.UserContext.AccountId == Guid.Empty ||
            !Enum.IsDefined(command.Request.EventType))
        ...
        if (command.Request.EventType == ManualOrderItemFulfillmentEventType.Failed && reason is null)
            return ApiResult<ManagementOrderDetailResult>.Fail("A failure reason is required.");
        var payloadHash = OrderItemFulfillmentIdempotency.ComputePayloadHash(
            command.Request.EventType.ToString(), reason);
        ...
        var result = await orders.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (item.FulfillmentType != FulfillmentType.Manual)
                return ApiResult<ManagementOrderDetailResult>.Fail(
                    "Manual fulfillment events apply only to manual order items.", 409);

            await orders.AcquireFulfillmentEventLockAsync(
                item.Id, command.Request.FulfillmentEventId, ct);
            ...
            if (order.PaymentStatus != PaymentStatus.Paid ||
                ...
                return ApiResult<ManagementOrderDetailResult>.Fail(
                    "Order item fulfillment can change only after payment and before the order becomes terminal.", 409);

            var fromItemStatus = item.Status;
            previousOrderStatus = order.Status;
            var changedAt = DateTimeOffset.UtcNow;
            try
            {
                ApplyItemEvent(item, command.Request.EventType);
                OrderFulfillmentAggregator.Apply(order, changedAt, reason);
            }
            ...
            {
                OrderItemId = item.Id,
                SourceEventId = command.Request.FulfillmentEventId,
                SourcePayloadHash = payloadHash,
                FromStatus = fromItemStatus,
            ...
            if (order.Status != previousOrderStatus)
                await orders.AddOrderStatusHistoryAsync(new OrderStatusHistory
                {
                    ...
                    ChangedAt = changedAt,
                    ChangedByAccountId = command.UserContext.AccountId,
                    Reason = $"Order item fulfillment changed to {item.Status}. {reason}".Trim()
                }, ct);
    ...
    private static void ApplyItemEvent(OrderItem item, ManualOrderItemFulfillmentEventType eventType)
    {
        switch (eventType)
        {
            case ManualOrderItemFulfillmentEventType.Accepted: item.MarkAccepted(); break;
            case ManualOrderItemFulfillmentEventType.Preparing: item.MarkPreparing(); break;
            case ManualOrderItemFulfillmentEventType.Completed: item.MarkCompleted(); break;
            case ManualOrderItemFulfillmentEventType.Failed: item.MarkFailed(); break;
            default: throw new DomainRuleException("Unsupported fulfillment event.");
        }
    }
