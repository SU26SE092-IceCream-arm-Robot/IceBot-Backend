...
- `KioskIngredientInventory` is the authoritative per-kiosk Ingredient balance. Dispenser state is optional topology/sensor evidence and must bind to an existing balance when present; a kiosk can operate a manual balance without dispenser hardware.
...
- Every `StockMovement` references its authoritative `KioskIngredientInventory`. `IngredientDispenserStateId` is optional evidence only. New movements persist nullable `BalanceBefore` in addition to `BalanceAfter`; a null before-value means the authoritative estimate was unknown, not that the movement lacks a balance owner.
...
- Refill, estimate adjustment, sensor baseline update, and execution consumption serialize on the authoritative kiosk-inventory mutation identity. Topology lifecycle operations additionally lock the affected dispenser evidence; multi-balance reports acquire locks in deterministic balance-id order.
...
- `GET /management/organizations/{organizationId}/configuration-releases/authoring-options` is a tenant-scoped UI lookup read model owned by configuration-release authoring. It returns eligible machine-produced ProductVariant options, Published/Active Recipes with their production-affecting option candidates, and organization-owned Published RobotPrograms with scope, display metadata, and declared workcell capability codes. `workcellCapabilities` is the deduplicated capability-code index with the published programs that declare each code. It does not invent display labels or capability versions; readiness does not report comparable versions. `productVariantId`, `search`, and `limit` are optional; `limit` applies independently to each result group. The command handler still revalidates every selected id when routes are submitted.
- Release route authoring requires Published/Active recipes to belong to their product variants and bindings to reference Published robot programs owned by the release organization. `requiredCapabilities` is a typed API collection of `{ code, required }`; `RequiredCapabilitiesJson` remains an internal persistence and runtime-manifest representation. Kiosk/device compatibility remains a deployment-time validation.
...
- Release lists return summaries without manifest JSON or route/binding collections. Release detail remains the review surface for the complete authored graph and returns its `revision`. Route replacement must echo that `expectedRevision`; a stale editor receives `409 Conflict` and must reload before replacing the aggregate route set.
...
- Deployment preview is kiosk-owned and read-only. Full Edge preview always includes the complete release and rejects route/program selections because the Full Edge command installs the whole release. Low-cost preview may accept route/program selections; otherwise it derives the only binding for each route and reports `ProgramSelectionRequired` for ambiguous routes. It evaluates active endpoint identity, readiness, safety/activity, reported capabilities, declared runtime/model against the Edge-reported device inventory, inventory policy, low-cost capacity, immutable artifact totals, installation modes, risk acknowledgement, and a deterministic preview checksum. A known profile mismatch is a blocker; no reported inventory is the non-blocking `RuntimeProfileUnknown` warning for the MVP. This gate does not inspect or certify Lua behavior. Preview never creates a deployment, Edge command, presigned URL, or Full Edge bundle.
...
- Production package routes require option codes to be unique within each packaged Product and at least one distinct required capability code per route. Package replace, publish, preview, install, admission, and dispatch preserve and enforce the complete capability set.
...
- Workspace actions are typed guidance only. Their structured context carries required parent resource IDs and, for deployment, the operationally routable endpoint/profile and low-cost route/program selections. Menu context separates assigned variant IDs from currently sellable variant IDs, so an existing inactive assignment produces deterministic activation/review guidance rather than duplicate assignment. Writes still use the owning command APIs. Installation-specific writes are `retry`, which reuses the persisted selection and original idempotency identity; `fork`, which changes technical ownership and copy-on-writes RobotArtifacts still shared with another package-managed installation; and `repair`, which restores soft-deleted materialization targets under their original identities. Fork never rewrites a Published program manifest: when a shared artifact replacement is needed, it creates and retargets a new Draft program while leaving the published program and existing release immutable.
...
- Technical workspace readiness means the release is published, operationally routable to an active endpoint, inventory topology satisfies base Recipe and required-option-group requirements, and the release is Active on the kiosk. Runtime-target/model metadata does not prove Lua compatibility. `latestDeploymentStatus` is not treated as informational-only readiness evidence.
- Robot artifact technical contracts are optional typed, versioned operator declarations. Metadata and Lua file names are not behavior authority. Referenced declarations are checked only for existence, lifecycle, scope, and checksum integrity; this does not prove that a declaration describes Lua. Artifact/template publication remains valid without a declaration after object integrity verification.
...
- Deployment validation preview returns a checksum and residual-risk warnings. Deploy requests must echo the current checksum and organization acknowledgement. Artifact integrity, endpoint scope/lifecycle, credentials, command persistence/delivery, declared capability, and topology failures remain blocking; runtime-target/model metadata and Lua semantic declarations are not Cloud compatibility gates.
