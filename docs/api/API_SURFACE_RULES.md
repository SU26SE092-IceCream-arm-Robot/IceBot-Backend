...
- Execution route `RequiredCapabilitiesJson` is optional internal persistence and runtime-manifest data. Management route authoring instead uses typed `requiredCapabilities[]` entries with `code` and `required`; requirement codes must be declared by that route's robot bindings. The management editor does not author `minVersion` because endpoint readiness does not report comparable capability versions. Existing internal/package contracts that contain a required `minVersion` remain fail-closed: the route is unavailable to runtime-menu and checkout and deployment/dispatch report `CapabilityVersionUnverifiable`; it is never silently ignored.
