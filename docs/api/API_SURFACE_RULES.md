...
- Online sales require `KioskStatus.Active`, active parent tenant scope, and a current kiosk connectivity projection of `Online` or `Degraded`.
- `KioskStatus` is lifecycle state. Connectivity is a separate observed projection and never changes lifecycle automatically.
...
- If more than one provider session for the same Order is confirmed paid and the accumulated amount exceeds the Order total, all payment evidence is retained but the Order becomes `RefundRequired`; fulfillment is not dispatched for the overpayment event.
...
- Fulfillment remains line-atomic in V1: one transition applies to the full `OrderItem.Quantity`. Partial unit handoff for manual or packaged lines is not supported.
...
- `GET /api/v1/management/payment-session-interventions` is a tenant-filtered `payments.manage` work queue. It returns bounded payment/order identity, issue code, retry state, and eligibility; it excludes raw provider payloads.
...
- `KioskStatus` owns management lifecycle (`Provisioning`, `Active`, `Maintenance`, `Disabled`, `Retired`). `KioskConnectivityProjection` separately owns observed connectivity (`Unknown`, `Online`, `Degraded`, `Unreachable`). Heartbeats and timeout reconciliation never mutate lifecycle.
...
- Order-level reports update only `OrderExecutionRecord`. Job-level reports require immutable `sourceProductionJobId`, `orderItemId`, `productionUnitNo`, and `productionUnitQuantity`; they update only the matching `ProductionExecutionRecord`. A source job cannot be rebound to a different order item or production unit. Stock evidence is job-scoped. The ingestion coordinator keeps one database transaction while deployment, order/job projection, and stock persistence use separate aggregate ports.
...
| `OperationsHub` | `/hubs/operations` | Kiosk & telemetry status | `OrderItemFulfillmentChanged`, `KioskStatusChanged`, `ExecutionReadinessChanged`, `DeviceEventCreated`, `AlertChanged`, `MaintenanceTicketChanged`, `InventoryChanged` |
