...
-> open Kiosk configuration workspace
-> create/review/publish a release
...
| 2A. Stage authoring bundle | Management UI | `POST /api/v1/management/organizations/{organizationId}/robot-authoring-imports` | Uploads one bounded ZIP with `Idempotency-Key`, validates archive structure and target consistency, and creates a durable organization-scoped import session. For a valid production-aware bundle, the same request automatically validates and materializes Draft technical resources when the actor also has `program.manage`. It never publishes, creates a release, or deploys. Retry with the same idempotency key resumes only unfinished automatic validation/materialization. |
| 2AL. Import raw Lua into Draft program | Management UI | `POST /api/v1/management/organizations/{organizationId}/robot-programs/{programId}/raw-lua-artifacts` | Compatibility path for one raw ZIP containing only `.lua` entries, or one to fifty individual `.lua` files. Requires `artifact.upload` and `program.manage`; accepts only a Draft program and appends successful artifacts after its current maximum `RunOrder`. A raw ZIP keeps ZIP entry order; individual files keep multipart order. The UI may prefill runtime target and machine model only when every existing artifact in the selected Draft has the same target pair; it must not hard-code a global machine default. Raw artifacts are created as opaque Draft artifacts with no technical contract, so they cannot be published or satisfy Recipe composition until a technical contract is authored and assigned. If any file upload fails, successful files remain unassigned Draft artifacts and the caller must correct/retry rather than receiving a partially reordered program. |
...
| 2C1. Adjust materialized Draft order | Management UI | `PUT /api/v1/management/organizations/{organizationId}/robot-programs/{programId}/artifacts` | Before composition confirmation or publication, an operator may drag artifacts into a preferred order. The request includes the full contiguous order and `expectedLastModifiedAt`; a stale editor receives `409`. The subsequent composition preview uses that current Draft order as its tie-breaker, but declared phases and before/after constraints remain mandatory and may reorder or block the proposal. |
...
| 2P. Confirm import publication | Management UI | `POST /api/v1/management/organizations/{organizationId}/robot-authoring-imports/{importId}/publish-resources` | Requires a previously confirmed exact Recipe/option composition. Explicitly publishes/reuses each contract, assigns it to the Draft artifact, verifies and publishes each artifact, then publishes the ordered program. The operation is resumable and stops with the exact resource error; it never creates a release or deployment. |
| 2R. Create release draft from import | Advanced management automation | `POST /api/v1/management/organizations/{organizationId}/robot-authoring-imports/{importId}/create-release-draft` | Retained typed orchestration API for an explicit automated workflow. It requires both import publication and the same confirmed Recipe/option composition, then derives the Published program, route code, capability JSON, priority, and binding order. The normal authoring UI does not call it: release creation, endpoint selection, and deployment belong to the Kiosk configuration workspace. |
| 2W. Read authoring workspace | Management UI | `GET /api/v1/management/organizations/{organizationId}/robot-authoring-imports/{importId}/workspace` | Aggregates import progress, package-installation ownership targets, `recipeResolution`, composition blockers, and authoring next actions. `recipeResolution` is `SingleMatch`, `MultipleMatches`, `NoMatch`, `OptionSelectionRequired`, or `NotReady`. Matching compares all non-optional Recipe ingredients with fixed V2 artifact quantities exactly; it is recommendation only. A single match may be preselected by the UI but still needs composition confirmation. No match, multiple matches, or option-specific effects stop the automatic workflow at Draft materialization and appear as workspace blockers; they never cause automatic publish, release, or deployment. Package ownership is informational. The workspace does not automatically require or propose a fork; a separate explicit customization workflow decides whether package-managed technical resources must be forked before mutation. |
