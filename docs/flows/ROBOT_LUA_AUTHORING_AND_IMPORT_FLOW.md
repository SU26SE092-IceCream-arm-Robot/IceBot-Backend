...
It is the convergence read model for import status, structural validation,
optional Recipe suggestions, and currently allowed import actions. The client
must use returned actions as guidance but
...
| --- | --- | --- |
| `ResumeImport` | `POST .../{importId}/resume` | Recovery only: one operator action resumes structural validation and Draft-resource materialization after an interrupted or blocked import. The normal upload already performs both operations automatically. |
...
Recipe binding, release authoring, and kiosk deployment are outside this import
workspace and use their own typed APIs.
...
`POST /api/v1/management/organizations/{organizationId}/production-program-bindings` accepts the Recipe, Published RobotProgram, and supported production-option codes. Creating the binding is the operator confirmation boundary. Backend snapshots capability proposals from optional published declarations into `requiredCapabilityCodes`; these are operator declarations, not inferred Lua requirements. When every artifact declares the built-in `FAIRINO_LUA_V1` / `FR5` packaging profile and no capability declaration exists, Backend adds the default `ROBOT_ARM` with evidence status `TargetProfileDefault`. This only declares the packaging profile; it does not prove Lua behavior, installed hardware, or runtime availability. Other missing declarations remain without a capability requirement.
