...
- **Persistence**: PostgreSQL 17 via EF Core (`IceBotDbContext`). The merged source contains 100 `DbSet<T>` declarations, eight non-designer migrations, and 101 cumulative `CreateTable` operations; these are static source counts, not a verified live-schema count. New persisted concepts include Inventory Sensor Observations and Production Program Bindings. Binary robot artifacts (`.lua` files) remain in MinIO rather than PostgreSQL. Evidence: `backend_update_impact_2026-08-11.md` §4. `[Open Question]` Live-schema reconciliation remains outstanding.
...
Post-sync scope additions include authenticated current-session management, persisted Edge inventory sensor observations, and operator-confirmed Recipe-to-RobotProgram production bindings. Runtime-menu caching, raw Lua import, deployment audit/concurrency, and CI/CD changes refine existing capabilities rather than adding separate product domains. Evidence: `backend_update_impact_2026-08-11.md` §§1–6.
...
- `[Open Question]` The pre-sync database inventory retains obsolete counts. The merged source currently has 100 DbSet declarations, eight migrations, and 101 cumulative CreateTable operations; exact live-schema reconciliation is still required.
