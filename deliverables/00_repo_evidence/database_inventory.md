...
The merged `IceBotDbContext` exposes 100 `DbSet<T>` declarations. This is a static source count, not a verified physical/live-table count. Grouped by bounded context (folder under `src/Domain/`):
...
**Evidence**: current `src/Infrastructure/Data/IceBotDbContext.cs`; migration table names confirmed through the eight non-designer migrations listed in §7. Exact entity/table reconciliation remains `[Needs Team Review]` under EFX-003.
...
- **Migration history**: eight non-designer migrations exist in current source. The five pre-sync migrations are followed by `20260731040709_AddInventorySensorObservations`, `20260804031725_AddProductionProgramBindings`, and `20260809035315_DeriveProductionBindingCapabilitiesFromContracts`. Across all eight files there are 101 cumulative `migrationBuilder.CreateTable` calls. The 100 DbSet declarations, eight migration files, and 101 creation calls are different static measures and are not a verified current/live-table count. `[Needs Team Review]` Model-snapshot/live-schema reconciliation remains outstanding.
- **Post-sync migration semantics**: `AddInventorySensorObservations` adds persisted inventory-observation evidence and its constraints/indexes. `AddProductionProgramBindings` adds production-program bindings and optional route-binding snapshot references/fields. `DeriveProductionBindingCapabilitiesFromContracts` derives binding capability evidence from existing declarations and converts historical route required-capability storage from the prior scalar representation to JSON arrays. Existing binding rows use the migration's missing-evidence default; this inventory does not claim that the migrations were executed against a live environment. Evidence: the three named migration and designer files, current model snapshot, and `backend_update_impact_2026-08-11.md` §4.
- **Manual migration steps**: two earlier migrations carry hand-written raw-SQL manual-step classes, including the provider-payment identity preflight. Their presence does not prove execution.
...
**Evidence**: `docker/docker-compose.yml`; `src/Infrastructure/AppDbContextFactory.cs`; `src/Infrastructure/DependencyInjection.cs`; `src/Infrastructure/Data/IceBotDbContext.cs`; EF configuration/convention files; `src/Infrastructure/Migrations/**` (eight non-designer migrations, designer files, manual steps, and `IceBotDbContextModelSnapshot.cs`); `backend_update_impact_2026-08-11.md` §4; MinIO artifact storage implementation.
