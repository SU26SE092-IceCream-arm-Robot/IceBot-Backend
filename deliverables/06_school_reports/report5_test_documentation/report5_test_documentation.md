...
The proposed scope follows the 135 functional requirements in the synchronized Report 3 and RTM. The high-level coverage areas are:
...
| Backend-sync additions | FR-134–FR-135 | Inventory sensor-observation ingestion and Production Program Binding lifecycle. | `[Planned]` |
...
|---|---|---|---|---|---|
| FR-001–FR-025, FR-027–FR-119, FR-128–FR-135 | In Scope | Backend-owned behavior is represented by repository evidence. Broad cases must be split into executable requirement/subclaim variants. | Unit/Integration/System as assigned in STM | `[Needs Team Review]` | Approved backend/database baseline; external/client dependency where applicable. |
...
| TC-IDN-003 | FR-003–FR-006, NFR-009, NFR-011 | Session/password lifecycle | Integration | Security | Active accounts, refresh/reset tokens, controlled clock. | Refresh; list owned sessions; revoke one/current/all sessions; attempt cross-account revocation; reset/change password; retry invalid tokens. | Only owned valid sessions/tokens succeed; metadata/current marker and credential-triggered revocation match the contract; UI privacy rules remain `[Needs Team/UI Review]`. | High | `[Planned]` |
...
| TC-IDN-008 | FR-009–FR-015 | Organization-owned account administration | Integration | Functional / API / authorization | SystemAdmin, OrgAdmin, two organizations, and target accounts. | Exercise organization-owned routes and assignable-role options; attempt Manager, cross-organization, and OrgAdmin-to-SystemAdmin operations. | Only SystemAdmin or authorized OrgAdmin operations within the route organization succeed; OrgAdmin cannot grant SystemAdmin. | High | `[Planned]` |
...
| TC-SC-001 | FR-042–FR-047, NFR-013 | Menu and runtime projection | System | Functional / cache | Scoped catalog, menu, kiosk, configuration, and cache data. | Request before/after admission changes; hit/miss/expire/invalidate the cache; inject cache failure; exercise ETag/304. | Admission is evaluated before cache access; cache failure falls back to the database projection; request metadata and deterministic revision follow the contract. | High | `[Planned]` |
...
| TC-INV-003 | FR-134 | Inventory sensor observations | Integration | Edge contract / idempotency / database | Authenticated endpoint, calibrated dispenser, controlled sequences and clock. | Submit newer, duplicate, conflicting-event, stale, cross-kiosk, invalid-level, and oversized batches over MQTT. | Evidence persists idempotently; only newer observations update projection; no stock movement or consumption claim is created. | High | `[Planned]` |
...
| TC-PAY-002 | FR-070, NFR-012 | PayOS webhook | System | Payment / security | Deterministic signed callback generator and eligible/unmatched references. | Send valid matched, verified-unmatched, invalid-signature, duplicate, late, and conflicting callbacks. | Invalid signatures and verified-unmatched callbacks create no payment/order/fulfilment state; unmatched diagnostics are bounded; matched transitions remain idempotent. Late/conflict precedence is `[Needs Team Review]`. | Critical | `[Planned]` |
...
| TC-IOT-001 | FR-120 | Edge command pull | Integration | Edge contract / security | Authenticated endpoint with pending commands for schemas 3–5. | Pull within/max limit; exercise repeated/wrong endpoint requests and scalar-to-array compatibility fixtures. | Only eligible commands are returned; schema v5 capability arrays and Cloud decoding of schemas 3–5 match the contract. | Critical | `[Planned]` |
...
| TC-ROBOT-001 | FR-088–FR-101, NFR-022 | Robot artifacts/programs | System | Functional / storage | Isolated MinIO and authorized configuration user. | Exercise optional declarations and raw-Lua Draft import, lifecycle operations, invalid/oversized input, and DB/object-store partial failures. | Binary integrity gates apply without treating optional declarations as certification; failed raw imports do not partially alter membership. | High | `[Planned]` |
| TC-ROBOT-002 | FR-097, FR-100 | Robot-program concurrency and import listing | Integration | Functional / concurrency / authorization | Draft program, two observed `LastModifiedAt` values, organization-scoped authoring imports, isolated object storage. | Replace artifact order with matching then stale `expectedLastModifiedAt`; import invalid/oversized raw files and inject storage/database failure; list/filter imports from owning and foreign organizations. | Matching revision updates atomically; stale revision conflicts without overwriting membership; invalid or failed import leaves no partial membership; listing is filtered, paged, and organization-scoped. | High | `[Planned]` |
...
| TC-PC-002 | FR-135, FR-102–FR-110 | Production binding and deployment concurrency | Integration | Functional / audit / concurrency | Recipe, Published RobotProgram, optional declaration states, and concurrent release/deployment revisions. | Create/list/retire bindings; try invalid options/scope; race route replacement/deployment; inspect reason/audit snapshots. | Unique immutable binding evidence and concurrency guards hold; missing declarations create no capability claim; records do not certify physical outcomes. | Critical | `[Planned]` |
...
| TC-OPS-002 | FR-082–FR-083 | Maintenance assignee options and revalidation | Integration | Functional / authorization / concurrency | Ticket with derived organization/store/kiosk scope; Active and inactive Technician/Manager candidates in matching and foreign scopes. | Query assignee options; vary role, active status, and scope; change eligibility after lookup; attempt assignment with the formerly eligible and cross-scope candidates. | Only currently Active eligible Technician/Manager candidates in the ticket-derived scope are returned, and assignment revalidates eligibility at mutation time; stale, inactive, or cross-scope assignment is rejected. | High | `[Planned]` |
...
|---|---:|---:|---:|---:|---|
| FR-001–FR-135 | 135 | `[Needs Team Review]` | `[To Be Updated After Test Execution]` | `[To Be Updated After Test Execution]` | `[To Be Updated After Test Execution]` |
