...
The kiosk runtime-menu projection shall support an optional bounded cache with ETag/`If-None-Match`. Store/kiosk sales admission shall be evaluated before cache access; request-specific `SnapshotId`/`GeneratedAt` metadata shall not be reused from the cached projection; cache disabled, miss, or failure shall fall back to the authoritative database projection; and checkout shall revalidate current sellability independently. Evidence: `backend_update_impact_2026-08-11.md` §§2, 5; synchronized baseline SRS NFR-013; current runtime-menu cache/projection implementation. Status: Supported for the optional cache boundary, admission ordering, fallback, metadata separation, and conditional-request contract. `[Needs Team Review]` Approved cache profile, TTL, invalidation settings, performance thresholds, and alert thresholds.
