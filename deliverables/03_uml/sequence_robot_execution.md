...
- MQTT Edge uplink consumption now covers 7 message types, adding `inventory-observations` to heartbeat/telemetry/readiness/execution-report/production-events/state-summaries. Inventory observations use the owning Inventory handler rather than an equivalent management REST write: `backend_update_impact_2026-08-11.md` §6; `srs.md` FR-126, FR-134.
...
- Execute-order payload schema v5 carries required capability-code arrays per program binding; Cloud decoding remains compatible with schemas 3–5. External Edge rollout compatibility is `[Needs Review]`: `backend_update_impact_2026-08-11.md` §6; `srs.md` FR-120.
